...
function updateTodayDate() {
    // Date display removed from header in v0.2.0 ("less is more" UX overhaul)
    // This function is kept for backward compatibility and potential future use
    // Week number calculations still available via getWeekNumber()
    return getWeekNumber(new Date(), CONFIG.weekStartsOnMonday);
}
...
/**
 * SVG icon helpers for assistant messages
 */
...
const assistantMessages = {
    focus: (taskCount, focusCount) => {
        ...
        // No tasks yet
        if (taskCount === 0) {
            return `Welcome! Add a task and click the ${assistantIcons.disk()} save icon to get started.`;
        }
        ...
        if (assistantState.justAddedFirstTask || (taskCount === 1 && focusCount === 0)) {
            return `Click the ${assistantIcons.star()} <strong>star icon</strong> to move tasks to Today's Focus. <strong>Swipe left</strong> to archive, <strong>swipe right</strong> to move to backlog. <strong>Drag up/down</strong> to reorder tasks.`;
        }

        // Has tasks but no focus
        if (focusCount === 0) {
            return `Click the ${assistantIcons.star()} <strong>star icon</strong> on tasks to move them to Today's Focus. <strong>Swipe left</strong> to archive, <strong>swipe right</strong> to move to backlog.`;
        }

        // Has focus tasks
        return `Great! Work on your focus tasks one at a time. <strong>Swipe left</strong> when done, or <strong>swipe right</strong> to move to backlog. <strong>Drag</strong> to reorder.`;
    },
    ...
    backlog: (backlogCount) => {
        ...
        if (backlogCount === 0) {
            return 'Your backlog is empty. Move tasks here from Today view when you need to defer them.';
        } else {
    ...
    archive: (archiveCount) => {
        if (archiveCount === 0) {
            ...
            return `Well done! You've completed ${archiveCount} task${archiveCount === 1 ? '' : 's'}. Keep building that momentum!`;
        }
    },
...
// Initialization is now handled in the single DOMContentLoaded listener above (line 22)
